...
const Cards = ({
  ...
  if (shouldAnimate) {
    if (smallScreen) {
      initialAnimation = { opacity: 0, x: 400 };
      animateAnimation = { opacity: 1, x: 0 };
      delete exitAnimation.y;
      exitAnimation.x = 400;
    } else {
      initialAnimation = { opacity: 0, y: 400 };
      animateAnimation = { opacity: 1, y: 0 };
      delete exitAnimation.x;
      exitAnimation.y = 400;
    }
  }
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 1, ease: "easeIn" }}
      exit={exitAnimation}
      className=" w-[250px] bg-gradient-to-l bg-[#110D2E]  from-[#110D2E] via-[#110D2E] to-[#fc466a4a] shadow-lg rounded-xl p-3"
    >
      <div className="flex flex-col" key={"caseStud_" + key}>
